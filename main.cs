...
namespace SA_MAP
{
    public partial class main : Form
    {
        ...
        {
            map.toggleCross();
            locationLab.Text = "ปิด/เปิดเส้นตำแหน่ง: " + (map.crossEnabled() ? "เปิดใช้งาน" : "ปิดใช้งาน");
        }
        private void saveAsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("โซนที่จะส่งออก", "เกิดข้อผิดพลาดในการส่งออก");
            }
        }
        private void openToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (map.getZones().Count > 0)
            {
                if (MessageBox.Show("คุณแน่ใจหรือไม่ว่าต้องการเปิดไฟล์ใหม่ การเปลี่ยนแปลงที่ยังไม่ได้บันทึกจะสูญหายไป", "การเปลี่ยนแปลงที่ไม่ได้บันทึก", MessageBoxButtons.OKCancel) == DialogResult.OK)
                {
                    open_main();
                }
            }
